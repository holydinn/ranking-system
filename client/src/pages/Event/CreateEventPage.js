...
const CreateEventPage = observer(() => {
  ...
  const handleNumberExpertChange = (event) => {
    setNumExperts(parseInt(event.target.value))
  };

  const handleNumberAltChange = (event) => {
    setNumAlts(parseInt(event.target.value))
  };
    ...
    try {
      ...
      expert.map((input) => {
        if (!input || input == '' || expert.length !== numExperts) {
          ...
          throw new SyntaxError('Вы не все имена экспертов!')
        }
      })
      alt.map((input) => {
        if (!input || input == '' || alt.length !== numAlts) {
          //alert('Вы не все имена участников!')
          throw new SyntaxError('Вы не все имена участников!')
        }
      })
      addEvent()
    } catch (e) {
  ...
  const addEvent = () => {
    try {
      ...
      createEvent({name: eventName, adminId: user.user.id}).then(data => {
        event.setEvents(data)
        curEvent = data
        console.log(curEvent)
      })
        .then(data => {
          expert.map((input) => (createExpert({name: input, eventId: curEvent.id}).then(data => {
            event.setEvents(data)
          })))
        })
        .then(data => {
          alt.map((input) => (
            createAlternative({name: input, eventId: curEvent.id}).then(data => {
    ...
    <Form>
      ...
      <Form.Group as={Row} className="mb-3">
        ...
        <Col sm="7">
          <Form.Control
            type="number"
            onChange={handleNumberExpertChange}
            required
      ...
      <Form.Group as={Row} className="mb-3">
        ...
        <Col sm="7">
          <Form.Control
            type="number"
            onChange={handleNumberAltChange}
            required
